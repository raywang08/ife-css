    ...
    if(this.positionFlag == 0) {
      ...
      setTimeout(() => {
        ...
        customEffective.every(ele => {
          if(ele.tableNum === tableNum) {
            // console.log('这儿是让顾客付款开始');
            ele.pay();
            // console.log('这儿是让顾客付款结尾');
            return false
          }
          return true
        })
      ...
      customEffective.every(ele => {
        if(ele.tableNum === tableNum) {
          // console.log('这儿是让顾客付款开始1');
          ele.pay();
          // console.log('这儿是让顾客付款结尾1');
          return false
        }
        return true
      })
...
Customer.prototype.pay = function () {
  ...
  // console.log('这儿是index');
  // console.log(index);
  customEffective.splice(index, 1);
  if (queues.length > 0) {
    // console.log('这儿是length>0')
    // 让顾客列表的第一个进入餐厅
...
Customer.prototype.come = function () {
  ...
  if(customEffective.length < 3) {
    customEffective.push(Object.assign({}, this));
  }

  console.log('这儿是come里面的customEffective');
  customEffective.forEach(ele => {
    console.log(ele);
  })
  // 替换对应的桌号上面的图片和名字
  ...
  // customName.innerText = 12321341234;
  // 然后通知服务员来点菜
  
  addEvent('order', this.tableNum);
}
/**
 * 顾客接受食物
 */
  ...
  for (let i = 0, len = menu.length; i < len; i++) {
    ...
    for (let j = 0, len1 = cookMenu.length; j < len1; j++) {
      if (cookMenu[j].id == menu[i].id) {
        ...
        console.log('这儿是去重的tableNum');
        console.log(cookMenu[j].tableNum);
        mark = false;
        break;
      }
    }
